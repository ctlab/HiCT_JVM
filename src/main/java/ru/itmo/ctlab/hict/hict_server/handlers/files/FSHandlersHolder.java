  ...
  public void addHandlersToRouter(final @NotNull Router router) {
    router.post("/list_files").blockingHandler(ctx -> {
      ...
      try (final var fileStream = Files.walk(dataDirectory)) {
        files = fileStream.filter(Files::isRegularFile).map(dataDirectory::relativize).map(Object::toString).filter(p -> p.endsWith(".hict") || p.endsWith(".hict.hdf5") || p.endsWith(".agp") || p.endsWith(".fasta") || p.endsWith(".fa") || p.endsWith(".cool") || p.endsWith(".mcool")).collect(Collectors.toList());
