...
** `TRADITIONAL_NORMALIZED` follows the active visualization settings, including `Observed`, `Expected`, and `O/E` display modes
** `Expected` and `O/E` use a distance-expected signal estimated from the currently fetched tile/query window after the selected raw-or-balanced normalization steps
...
HiCT release builds can produce self-contained desktop/CLI packages for users who do not have a recent Java installation.
These packages include the HiCT JVM fat JAR, the built HiCT WebUI resources, and a Java runtime image created with `jlink`.
...
* `HiCT-<version>-windows-x86_64-installer.exe` -- optional Windows installer produced by the official JDK `jpackage` tool.

The Windows portable artifact is intentionally a ZIP, not a custom self-extracting executable.
A true one-file portable Windows EXE requires an opaque self-extractor or custom packer, which is exactly the packaging style that often creates antivirus and redistribution-review problems.
The `.exe` artifact is therefore a standard `jpackage` installer; the ZIP is the portable Windows distribution.
...
When no CLI arguments are provided, HiCT starts API + WebUI and opens `http://localhost:8080/` in the default browser when desktop browsing is available.
All normal HiCT CLI subcommands remain available.
...
.\scripts\portable\build_portable_windows.ps1 -CreateInstaller
----
...
The workflow can also be run manually with toggles for bundled `hictk` and the optional Windows `jpackage` installer.
