...
It opens a console window and keeps it open after no-argument double-click launches, so errors remain visible after HiCT exits.
...
The Windows portable launcher also starts a small browser-opener helper so browser launch does not depend on Java Desktop integration.
