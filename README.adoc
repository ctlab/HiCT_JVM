...
**NOTE:** prebuilt native bundles are currently provided for *Windows* (tested on 10/11), *Linux x86_64 static-musl* binaries, and *macOS arm64* (Apple Silicon). The Linux and macOS portable artifacts are built to run without the host libc/stdlib dependencies that previously blocked Ubuntu 18.04/22.04 and Alpine users. On Windows you might need to install https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist?view=msvc-170[Microsoft Visual C++ Redistributable].
