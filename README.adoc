...
The supported bundled browser payload is the HiCT_WebUI Electron wrapper, which acts only as a local browser for the JVM-served WebUI; the lightweight Java launcher remains the default process controller.

Build the Electron payload from a HiCT_WebUI checkout:
...
Electron/Chromium is intentionally optional because it materially increases portable artifact size; leave `bundle_electron_browser=false` in the manual workflow when a smaller system-browser-only release is required.
...
The workflow can also be run manually and exposes toggles for bundled `hictk`, the bundled Electron browser, the `hictk` release tag, the HiCT_WebUI ref, and the optional Windows SFX EXE.
