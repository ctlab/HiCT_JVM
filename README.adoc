...
= Launching pre-built version (Easiest way to try HICT yourself) 

⚠️ The main way to launch HiCT is to **use pre-built** .exe file (for Windows) or .run file (for Linux). 
These are available for download in the **Releases section**: https://github.com/ctlab/HiCT_JVM/releases/tag/latest-master. Download files that either have .exe or .run extension, and simply run them with double-click (on Linux use `chmod +x` to allow execution). This way, you won't need to install any additional dependencies or external tools, as other distributions. 

= Advanced ways to start

Most users should download a ready-to-run package from the https://github.com/ctlab/HiCT_JVM/releases[GitHub Releases page].
You do not need to clone this repository or build the project unless you want to develop HiCT itself.

Open the Releases page, choose the newest release, scroll to *Assets*, and download the file for your operating system.

=== What should I download?

* *Windows:* download `HiCT-<version>-windows-x86_64.exe`.
  At first start, Windows might display a window "Windows protected your PC" and advise you not to run this file. That happens because HiCT does not have a digital signature now. Usually you can click on "More info" and the button "Run anyway" would appear.
  If Windows or your organization blocks unsigned `.exe` files, download `HiCT-<version>-windows-x86_64-portable.zip`, unzip it, and run `HiCT.cmd`.
* *Linux, normal desktop/server distributions:* download `HiCT-<version>-linux-x86_64.run`.
  This is the recommended choice for recent versions Ubuntu, Debian, Fedora, RHEL, CentOS, openSUSE, Arch, and similar Linux systems.
* *Linux, older/minimal systems or containers:* download `HiCT-<version>-x86_64.AppImage`.
  Use this if the `.run` file does not start, if it reports a glibc/runtime error, or if you are using Alpine/musl or an old manylinux-like container.
* *macOS Apple Silicon (M1, M2, ...):* download `HiCT-<version>-darwin-arm64.run`.
* *macOS Intel (Core i3/i5/i7):* download `HiCT-<version>-darwin-x86_64.run`.
  This is not a `.dmg` installer: there is no application icon to drag into Applications. Download the `.run` file, open Terminal, go to the folder with the downloaded file, unblock it if macOS asks, make it runnable, and start it from Terminal.
...
bash HiCT-<version>-linux-x86_64.run
----

For older/minimal Linux systems, Alpine, or containers:

[source,bash]
----
chmod +x HiCT-<version>-x86_64.AppImage
./HiCT-<version>-x86_64.AppImage
----

If the AppImage reports a FUSE or mount error, use the extraction fallback:

[source,bash]
----
APPIMAGE_EXTRACT_AND_RUN=1 ./HiCT-<version>-x86_64.AppImage
----

After startup, open the WebUI at `http://localhost:8080`.

To choose the folder where HiCT looks for `.hict.hdf5`, `.mcool`, `.cool`, `.agp`, and `.fasta` files, set `DATA_DIR`:

[source,bash]
----
DATA_DIR=/path/to/your/data bash HiCT-<version>-linux-x86_64.run
DATA_DIR=/path/to/your/data ./HiCT-<version>-x86_64.AppImage
----

=== Linux on a remote SSH cluster

If HiCT is running on a remote Linux server or compute node, start it in server mode and forward both HiCT ports to your computer.
Run this command on your local computer:

[source,bash]
----
ssh -L 8080:127.0.0.1:8080 -L 5000:127.0.0.1:5000 user@cluster.example.org
----

After SSH connects, run HiCT on the remote machine with `start-server` explicitly:

[source,bash]
----
cd /path/to/HiCT
DATA_DIR=/path/to/your/data bash HiCT-<version>-linux-x86_64.run start-server
----

For the AppImage artifact, use:

[source,bash]
----
cd /path/to/HiCT
DATA_DIR=/path/to/your/data ./HiCT-<version>-x86_64.AppImage start-server
----

Keep the SSH session open while HiCT is running.
Then open `http://localhost:8080` in the browser on your local computer.
The forwarded API port is `http://localhost:5000`.

=== Windows quick start

Double-click the downloaded `.exe`, or start the server from Command Line/PowerShell:

[source,powershell]
----
.\HiCT-<version>-windows-x86_64.exe
----

Then open `http://localhost:8080`.

If you downloaded the `.zip`, unzip it first and run:
...
The macOS package is a `.run` file, not a `.dmg` installer.
Do not look for a drag-and-drop Applications window.
Open Terminal and run the downloaded file from the folder where it was saved.
...
----
cd ~/Downloads
chmod +x HiCT-<version>-darwin-arm64.run
./HiCT-<version>-darwin-arm64.run
----

For Intel Macs, use `darwin-x86_64` instead:

[source,bash]
----
cd ~/Downloads
chmod +x HiCT-<version>-darwin-x86_64.run
./HiCT-<version>-darwin-x86_64.run
----

If macOS says the file cannot be opened because it is from an unidentified developer or was downloaded from the Internet, unblock it first:

[source,bash]
----
xattr -d com.apple.quarantine ./HiCT-<version>-darwin-*.run
----

Then run the `chmod +x ...` and `./HiCT-....run` commands again.
After startup, open the WebUI at `http://localhost:8080`.

=== Command-line help

All portable packages accept the same HiCT commands:

[source,bash]
----
./HiCT-<version>-<linux/darwin>-<x86_64/arm64>.run
./HiCT-<version>-<linux/darwin>-<x86_64/arm64>.run --help
./HiCT-<version>-<linux/darwin>-<x86_64/arm64>.run start-server --help
./HiCT-<version>-<linux/darwin>-<x86_64/arm64>.run convert --help
----

Replace the file name with the `.AppImage`, Windows `.exe`, macOS `.run`, or `HiCT.cmd` that you downloaded.

== Advanced: fat JAR distribution

This section is for users who specifically need the legacy ready-to-run fat JAR from GitHub Releases.
The portable packages above are recommended for normal use because they include the matching Java runtime and native libraries.
...
**NOTE:** prebuilt native bundles are currently provided for portable Windows, Linux, and macOS release artifacts. The legacy fat JAR path can still require a compatible local Java installation and platform-specific native libraries. On Windows you might need to install https://learn.microsoft.com/en-us/cpp/windows/latest-supported-vc-redist?view=msvc-170[Microsoft Visual C++ Redistributable].
...
2. Confirm OS compatibility (Linux x86_64 static-musl or macOS arm64 as appropriate).
...
The default Windows EXE mode is a small custom `asInvoker` launcher built with the static MSVC runtime.
It embeds an official standalone 7-Zip/LZMA extractor plus the portable app payload, extracts into a stable `HiCT.portable\payloads\...` cache next to the EXE, and skips extraction on later launches when the content-addressed cache marker matches.
The legacy official 7-Zip/LZMA SDK SFX mode (`7zSD.sfx`, `7zS.sfx`, `7zS2con.sfx`, or `7zS2.sfx`) remains available from the release workflow/manual build switch.
No-argument double-click launches open the graphical HiCT launcher; server output is shown inside the launcher log panel.
Avoiding Windows trust prompts for newly downloaded single-file EXE artifacts still requires Authenticode signing with a trusted code-signing certificate; launcher implementation alone is not a reliable substitute.
Use the ZIP artifact when local policy forbids unsigned launchers.
...
The Linux `.run` uses an appended `tar.xz` payload for smaller single-file releases; the companion `.tar.gz` stays available for environments that prefer direct archive extraction.
AppImage builds use a static AppImage runtime and gzip-compressed SquashFS by default, so the AppImage can start on older or musl-based Linux systems before HiCT switches to its bundled glibc runtime.
Set `APPIMAGE_COMPRESSION=xz` only together with a compatible AppImage runtime when smaller artifact size is more important than old-system compatibility.
...
The lightweight Java launcher remains the default process controller.
...
The Tauri payload does not bundle Chromium.
It uses the operating system WebView runtime: WebKitGTK on Linux and Microsoft Edge WebView2 on Windows.
Portable launchers warn before startup when a Tauri payload is present but the expected Linux WebKitGTK libraries or Windows WebView2 runtime are not detected, including distribution-specific install commands.
This keeps the default `.run`, AppImage, and Windows EXE substantially smaller than Chromium-bundled releases while still providing a built-in WebUI path on machines with the standard WebView runtime.
...
The payload builder keeps only the `en-US` Chromium locale by default to control release size.
Set `HICT_ELECTRON_KEEP_LOCALES=en-US,fr,de` before running the payload builder if additional locales must be redistributed.
Set `HICT_SKIP_NPM_INSTALL=1` only for local rebuilds where the HiCT_WebUI `node_modules` tree is already known to match `package-lock.json`.
...
Generic browser binaries are intentionally not downloaded by the HiCT packaging scripts.
The Tauri payload builder uses the Rust/Tauri dependencies declared by HiCT_WebUI, stores Cargo dependency metadata in the payload, and relies on the user's OS WebView runtime license terms.
The Electron payload builder uses the Electron package already declared by HiCT_WebUI, preserves `electron/LICENSE` and `electron/LICENSES.chromium.html`, flips Electron security fuses when supported by the installed Electron version, and writes a payload manifest with size and SHA-256 tree digest.
...
Electron/Chromium is intentionally optional because it materially increases portable artifact size; leave `bundle_electron_browser=false` in the manual workflow unless a Chromium fallback must be bundled.
...
The workflow can also be run manually and exposes toggles for bundled `hictk`, the default bundled Tauri browser, the optional bundled Electron browser, the `hictk` release tag, the HiCT_WebUI ref, the optional Windows EXE, and the Windows EXE mode (`custom` by default, `7zip-sfx` for legacy official SFX packaging).
`bundle_tauri_browser` defaults to `true`; `bundle_electron_browser` defaults to `false`.
...
For workflow downloads it publishes aggregate Linux and Windows packages plus separate single-file runnable artifacts for Linux `.run`, Linux AppImage, and Windows `.exe`.
