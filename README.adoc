...
* `HiCT-<version>-windows-x86_64.exe` -- optional single-file Windows 7-Zip SFX launcher, not an MSI installer.

The Windows ZIP is the most transparent portable artifact.
The Windows EXE is built only from official 7-Zip SFX modules (`7zS.sfx` or `7zSD.sfx`) and runs the same portable app without installation.
...
* Windows SFX EXE: directory containing the EXE when the launcher can infer it from the parent process; otherwise the temporary extraction directory used by the official 7-Zip SFX module.
